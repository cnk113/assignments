...
class MinimizerIndexer(object):
    ...
    def __init__(self, targetString, w, k, t):
        ...
        map = {}
        for i in range(len(targetString)-w+1):
            ...
            map[(minmer,id)] = map.get((minmer,id),0) + 1
        minmers = []
        for key in map:
            if map.get(key) <= t:
                minmers.append(key)
...
class SmithWaterman(object):
    ...
    def getAlignment(self):
        ...
        aligned = []
        id = numpy.unvravel_index(self.matrix.argmax(),self.matrix.shape)
        current = self.matrix[id[0],id[1]]
        while current != 0:
            aligned.append((id[0],id[1]))
    ...
    def getMaxAlignmentScore(self):
        ...
        """
        # Code to complete
        return max(self.matrix.max(axis=0))
