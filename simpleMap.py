...
class SmithWaterman(object):
    def __init__(self, string1, string2, gapScore=-2, matchScore=3, mismatchScore=-3):
        ...
        When calculating the DP matrix use a banded formula which only computes a band of the matrix.
        Also instead of calculating the score at the position, calculate the difference between adjacent cells
        ...
        for i in range(1,len(string1)+1):
            for j in range(1,len(string2)+1):
                if string1[i-1] == string2[j-1]:
                    val = [self.matrix[i-1,j-1] + matchScore]
                else:
                    val = [self.matrix[i-1,j-1] + mismatchScore]
    ...
    def getAlignment(self):
        ...
        aligned = []
        current = numpy.unravel_index(self.matrix.argmax(),self.matrix.shape)
        value = self.matrix[current[0],current[1]]
        while value != 0:
            bp = self.backpointer[current[0],current[1]]
            if bp == 0:
                current = (current[0]-1,current[1]-1)
                aligned.append((current[0],current[1]))
            elif bp == 1:
